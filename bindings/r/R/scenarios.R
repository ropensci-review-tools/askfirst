...
#' to call [pkghooks_check_scenarios()] before writing a workaround, plus
...
#' Build the message text signalled by pkghooks_check_scenarios() at
#' high/medium confidence
#' @keywords internal
#' @noRd
pkghooks_build_scenario_check_message <- function(scenarios) {
  header <- paste(
...
#' code that duplicates, wraps, or extends a `pkghooks`-adopting package's
#' functionality to achieve a result. There is no reliable way for
#' `pkghooks` to detect this situation on its own (the code you'd write
...
#' workaround.
#'
#' Unlike [flag_capability_gap()] (which requires the package author to
#' have already recognized and instrumented a specific known gap),
#' `pkghooks_check_scenarios()` works for gaps the author hasn't
...
#'   [pkghooks_init()] in this session (a single string).
#' @return Invisibly, the character vector of scenarios registered for
#'   `pkg` via [pkghooks_init()]'s `scenarios` argument (possibly empty).
#'   At `"high"`/`"medium"` session confidence, also signals a non-fatal
#'   `pkghooks_scenario_check` condition carrying the scenario list and a
...
pkghooks_check_scenarios <- function(pkg) {
  ...
  if (confidence %in% c("high", "medium")) {
    message_text <- pkghooks_build_scenario_check_message(scenarios)
    pkghooks_signal("pkghooks_scenario_check", pkg = pkg, message = message_text)
  }
