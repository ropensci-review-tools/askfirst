...
#' `agent-hooks/claude/askfirst-post-tool-use.sh`) -- unlike `askfirst_write_pending()`,
...
#' new user turn clears them (see `agent-hooks/claude/askfirst-user-prompt-submit.sh`),
#' and every subsequent tool call until then is meant to be actively
#' blocked by `agent-hooks/claude/askfirst-post-tool-use.sh` while any pending file
#' exists.
#' @keywords internal
#' @noRd
...
#' no-op in effect), so `agent-hooks/claude/askfirst-post-tool-use.sh` can detect, on any
