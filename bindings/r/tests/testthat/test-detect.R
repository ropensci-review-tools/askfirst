test_that("askfirst_detect_tool matches a simple env_set signature", {
  withr::local_envvar(c(CLAUDECODE = "1"))
  result <- askfirst:::askfirst_detect_tool()
  expect_equal(result$key, "CLAUDE")
})

test_that("askfirst_detect_tool matches an env_value signature", {
  withr::local_envvar(c(CURSOR_EXTENSION_HOST_ROLE = "agent-exec"))
  result <- askfirst:::askfirst_detect_tool()
  expect_equal(result$key, "CURSOR_CLI")
})

test_that("askfirst_detect_tool matches an env_matches (regex) signature", {
...
test_that("askfirst_detect_tool returns NULL with no known signals present", {
