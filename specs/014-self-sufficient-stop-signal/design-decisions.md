...
**Extended by:** Stage 019 added a second, `askfirst_signal()`-based agent-directed `askfirst_hooks_nudge` condition, gated on high AI-agent confidence, signalled *alongside* (not instead of) this decision's human-directed nudge, which remains unchanged and still fires independent of confidence. A reader relying on this entry alone should not conclude the nudge is still human-directed-only.
