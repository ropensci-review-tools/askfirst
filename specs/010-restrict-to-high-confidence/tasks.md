...
- [x] T010-1: In `bindings/r/R/init.R:74`, change `confidence %in% c("high", "medium")` to `identical(confidence, "high")`, and update the roxygen doc on line 10 to remove `" or medium"`.

## T010-2: Restrict error handler to high confidence
- [x] T010-2: In `bindings/r/R/init.R:154`, change `!(confidence %in% c("high", "medium"))` to `!identical(confidence, "high")`, and update the roxygen doc on line 92 to remove `"high"/"medium"`.

## T010-3: Restrict scenario check to high confidence
- [x] T010-3: In `bindings/r/R/scenarios.R:116`, change `confidence %in% c("high", "medium")` to `identical(confidence, "high")`, and update the roxygen docs on lines 28 and 81 to remove `"high"/"medium"` references.

## T010-4: Restrict capability gap to high confidence
- [x] T010-4: In `bindings/r/R/capability_gap.R:48`, change `identical(confidence, "low")` to `!identical(confidence, "high")`, and update the roxygen doc on line 12.

## T010-5: Update tests for medium-confidence no-signal behaviour
- [x] T010-5: Update these tests to expect no signal (instead of expecting a signal) under medium confidence:
...
- [x] T010-6: Run the test suite (e.g. `devtools::test()` or `R CMD check`) and confirm all tests pass after the changes.
