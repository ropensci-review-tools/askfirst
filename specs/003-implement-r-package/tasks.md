...
- [ ] T003-1: Create a standard R package named `pkghooks` rooted at `r/`:
  ...
  `r/tests/testthat.R` + `r/tests/testthat/` scaffold, `r/inst/` (populated
  by T003-2), `r/.Rbuildignore`, and `r/NAMESPACE` (roxygen2-generated
...
- [ ] T003-2: Write a sync script (`r/data-raw/sync-vendor.R`) that copies
  ...
  `r/inst/agent-detect-spec/agents.schema.json`. Run it once now to
  populate `r/inst/agent-detect-spec/`. Add a CI check step (in the
  workflow from T003-12, or a standalone script) that fails if
  `r/inst/agent-detect-spec/` and `agent-detect-spec/vendor/` ever diverge
  byte-for-byte.
...
- [ ] T003-3: Modify `.github/workflows/sync-agent-detect-spec.yml` so
  ...
  `r/inst/agent-detect-spec/agents.json` and
  `r/inst/agent-detect-spec/agents.schema.json` in the same pull request's
...
- [ ] T003-7: Write `r/R/init.R` exporting
  ...
  `r/R/state.R` defining `.pkghooks_state <- new.env(parent =
...
- [ ] T003-10: Write `r/tests/testthat/test-detect.R` (using
  ...
  `r/tests/testthat/test-confidence.R` (asserting `"high"`/`"medium"`/
  `"low"` under simulated env-var-match / no-match-no-TTY / plain-TTY
  conditions), `r/tests/testthat/test-init.R` (asserting `pkghooks_init()`
  ...
  `r/tests/testthat/test-capability-gap.R` (asserting `flag_capability_gap()`
...
- [ ] T003-12: Write `.github/workflows/r-cmd-check.yml`: on push/PR
  ...
  `r/inst/agent-detect-spec/` differs from `agent-detect-spec/vendor/`).
